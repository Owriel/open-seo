...
function ReportPage() {
  ...
  // Pre-rellenar todos los campos desde el contexto del proyecto. "gbp"
  // (ficha de Google) acepta nombre o URL; usamos businessName o placeId.
  useEffect(() => {
    if (!project) return;
    ...
    if (gbp === "") {
      if (project.businessName) setGbp(project.businessName);
      else if (project.placeId) setGbp(project.placeId);
    }
  ...
  const reportMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setPublicUrl(
        data.publicId
          ? `${window.location.origin}/report/${data.publicId}`
          : null,
      );
    ...
    onError: (err) =>
      toast.error(getStandardErrorMessage(err, "Error generando informe")),
  });
  ...
  const publicLinkMutation = useMutation({
    mutationFn: () =>
      generatePublicLink({ data: { projectId, reportId: report!.id } }),
    onSuccess: (data) => {
  ...
  const disableLinkMutation = useMutation({
    mutationFn: () =>
      disablePublicLink({ data: { projectId, reportId: report!.id } }),
    onSuccess: () => {
      setPublicUrl(null);
  ...
  const handleLoadReport = async (reportId: string) => {
    try {
      ...
      if (result) {
        ...
        setPublicUrl(
          result.publicId
            ? `${window.location.origin}/report/${result.publicId}`
            : null,
        );
      }
    } catch (err) {
  ...
  const handleExportCsv = () => {
    ...
    for (const kw of report.visibility.topKeywords.slice(0, 50)) {
      rows.push([
        "Visibilidad",
        kw.keyword,
        `Pos ${kw.position} | Vol ${kw.searchVolume} | Tráf ${kw.traffic}`,
      ]);
    }
    for (const opp of report.opportunities.results.slice(0, 30)) {
      rows.push([
        "Oportunidades",
        opp.keyword,
        `Pos ${opp.position} | Score ${opp.score}`,
      ]);
    }
    for (const comp of report.competitors.competitors) {
      rows.push([
        "Competencia",
        comp.domain,
        `KWs ${comp.organicKeywords} | Tráf ${comp.organicTraffic}`,
      ]);
    }
    for (const kw of report.contentGap.keywords.slice(0, 30)) {
      rows.push([
        "Content Gap",
        kw.keyword,
        `Vol ${kw.searchVolume} | Pos comp ${kw.competitorRank}`,
      ]);
    }
    for (const issue of report.technical.issues) {
      rows.push([
        "Técnico",
        issue.description,
        `${issue.severity} | ${issue.page}`,
      ]);
    }
    downloadCsv(
      `informe-seo-${report.domain.replace(/\./g, "-")}.csv`,
  ...
  const sections: Array<{
    ...
    ? [
              ...
              <table className="table table-xs w-full">
                <thead>
                  <tr className="text-xs text-base-content/60">
                    ...
                    <th className="text-right">Tráfico</th>
                    <th className="text-right">Comunes</th>
                ...
                <tbody>
                  {report.competitors.competitors.map((c, i) => (
                    <tr key={i} className="hover:bg-base-200/30">
                      ...
                      <td className="text-right tabular-nums">
                        {formatNumber(c.organicKeywords)}
                      </td>
                      <td className="text-right tabular-nums">
                        {formatNumber(c.organicTraffic)}
                      </td>
                      <td className="text-right tabular-nums">
                        {formatNumber(c.commonKeywords)}
                      </td>
                      <td className="text-right tabular-nums">
                        {c.avgPosition}
                      </td>
              ...
              <table className="table table-xs w-full">
                <thead>
                  <tr className="text-xs text-base-content/60">
                    ...
                    <th className="text-right">Pos. competidor</th>
                  </tr>
                </thead>
                <tbody>
        ...
        },
        {
          ...
          summary: report.local
            ? `Posición: ${report.local.ourPosition ?? "No aparece"} · ${report.local.localKeywords.length} keywords locales`
            : "No analizado",
                  ...
                  <table className="table table-xs w-full">
                    ...
                    <tbody>
                      {report.local.localPackResults.map((r, i) => (
                        <tr
                          key={i}
                          className={`hover:bg-base-200/30 ${r.position === report.local!.ourPosition ? "bg-primary/10 font-semibold" : ""}`}
                        >
          ...
          render: () => (
            <div className="space-y-1">
              {["critical", "warning", "info"].map((sev) => {
                const items = report.technical.issues.filter(
                  (i) => i.severity === sev,
                );
                ...
                return items.map((issue, i) => (
                  <div
                    key={`${sev}-${i}`}
                    className="flex items-start gap-2 py-1 px-2 rounded hover:bg-base-200/30"
                  >
                    <span
                      className={`badge badge-xs ${getSeverityClass(sev)} mt-0.5`}
                    >
                      {sev}
                    </span>
        ...
        },
        {
          ...
          summary: report.gbp
            ? `${report.gbp.variants.length} variantes detectadas`
            : "No analizado",
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="shrink-0 px-4 md:px-6 pt-4 pb-2 max-w-8xl mx-auto w-full">
        ...
        <p className="text-sm text-base-content/60 mb-3">
          Genera un informe completo: visibilidad, oportunidades, competencia,
          salud técnica y presencia local.
        </p>
        <form
        ...
        >
          ...
          <input
            className="input input-bordered input-sm w-auto max-w-[200px]"
            placeholder="Keyword principal"
            value={keyword}
            onChange={(e) => setKeyword(e.target.value)}
          />
          <input
            className="input input-bordered input-sm w-auto max-w-[240px]"
            placeholder="Ficha Google (nombre o URL)"
            value={gbp}
          ...
          {reportMutation.isPending ? (
            <div className="mt-8 text-center space-y-4">
              ...
              <p className="text-sm text-base-content/60 animate-pulse">
                Analizando {domain}... Esto puede tardar 15-30 segundos.
              </p>
            </div>
              ...
              <div className="border border-base-300 rounded-xl bg-base-100 p-5">
                <div className="flex items-center gap-6 flex-wrap">
                  <div className="flex flex-col items-center">
                    <span
                      className={`text-4xl font-bold badge ${getScoreColorClass(report.scores.global.color)} badge-lg p-4`}
                    >
                  ...
                  <p className="text-sm text-base-content/70 flex-1">
                    {report.scores.global.summary}
                  </p>
                  <div className="flex gap-3 flex-wrap">
                    {sections
                      .filter((s) => s.show && s.score)
                      .map((s) => (
                        <div key={s.key} className="flex flex-col items-center">
                          <span
                            className={`badge ${getScoreColorClass(s.score!.color)} badge-sm`}
                          >
                            {s.score!.score}
                          </span>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium text-sm flex items-center gap-2">
                            ...
                            {s.score && (
                              <span
                                className={`badge badge-xs ${getScoreColorClass(s.score.color)}`}
                              >
                                {s.score.score}
                              </span>
                            )}
            ...
            <div className="mt-8 text-center space-y-3">
              ...
              <p className="text-lg font-medium text-base-content/70">
                Genera un informe SEO completo
              </p>
              <p className="text-sm text-base-content/50 max-w-md mx-auto">
                Introduce un dominio para analizar visibilidad, oportunidades,
                competencia, salud técnica y más.
              </p>
            </div>
          )}
          ...
          {savedReports.length > 0 && (
            <div className="mt-6">
              <h2 className="font-semibold text-sm mb-3 flex items-center gap-2">
                <Save className="size-4 text-primary" /> Informes guardados (
                {savedReports.length})
              </h2>
              <div className="space-y-2">
                {savedReports.map((s) => (
                  <div
                    key={s.id}
                    className="border border-base-300 rounded-lg bg-base-100 px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-base-200/30"
                    onClick={() => handleLoadReport(s.id)}
                  >
                    <div>
                      <span className="font-medium text-sm">{s.domain}</span>
                      <span
                        className={`badge badge-xs ${s.score >= 70 ? "badge-success" : s.score >= 40 ? "badge-warning" : "badge-error"} ml-2`}
                      >
                        {s.score}
                      </span>
                      <span className="text-xs text-base-content/40 ml-2">
                        {new Date(s.generatedAt).toLocaleDateString("es-ES")}
                      </span>
                    </div>
                    <button
