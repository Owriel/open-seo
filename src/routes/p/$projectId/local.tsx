    ...
    <div className="border border-base-300 rounded-lg bg-base-100 overflow-hidden">
      ...
      {isExpanded && (
        <div className="border-t border-base-300 px-4 py-3 bg-base-200/30 space-y-3">
          ...
          {/* Rating distribution */}
          {r.ratingDistribution && Object.keys(r.ratingDistribution).length > 0 && (
            <div>
              <span className="text-xs font-medium text-base-content/50 uppercase">
...
type SortField = "keyword" | "searchVolume" | "cpc" | "keywordDifficulty" | "intent" | "priority";
    ...
    arr.sort((a, b) => {
      switch (sortField) {
        ...
        case "keywordDifficulty":
          return dir * ((a.keywordDifficulty ?? -1) - (b.keywordDifficulty ?? -1));
        case "intent":
        ...
        default:
          return (
            dir *
            (calculatePriorityScore(a.searchVolume, a.keywordDifficulty, a.cpc) -
              calculatePriorityScore(b.searchVolume, b.keywordDifficulty, b.cpc))
          );
      }
    });
        ...
        <table className="table table-sm w-full">
          <thead>
            <tr className="text-xs text-base-content/60">
              <SortHeader label="Keyword" field="keyword" currentField={sortField} currentDir={sortDir} onSort={handleSort} />
              <SortHeader label="Volumen" field="searchVolume" currentField={sortField} currentDir={sortDir} onSort={handleSort} align="right" />
              <SortHeader label="CPC" field="cpc" currentField={sortField} currentDir={sortDir} onSort={handleSort} align="right" />
              <SortHeader label="Dificultad" field="keywordDifficulty" currentField={sortField} currentDir={sortDir} onSort={handleSort} align="center" />
              <SortHeader label="Intent" field="intent" currentField={sortField} currentDir={sortDir} onSort={handleSort} align="center" />
              {showLocalColumn && <th className="text-center">Local</th>}
              <SortHeader label="Prioridad" field="priority" currentField={sortField} currentDir={sortDir} onSort={handleSort} align="center" />
            </tr>
          </thead>
          <tbody>
            {sorted.map((kw, idx) => {
              const score = calculatePriorityScore(kw.searchVolume, kw.keywordDifficulty, kw.cpc);
              ...
              return (
                <tr key={idx} className="hover:bg-base-200/50">
                  ...
                  <td className="text-center">
                    <span className={`badge badge-sm ${priorityTierClass(tier)}`}>
                      {score}
                    </span>
                  </td>
                </tr>
...
function LocalPage() {
  ...
  const [activeTab, setActiveTab] = useState<"pack" | "analysis" | "keywords">("pack");
  const [kwSubTab, setKwSubTab] = useState<"withCity" | "withoutCity">("withCity");
  ...
  const cityKeywordsQuery = useQuery({
    queryKey: ["localCityKeywords", baseKeyword, detectedCity, searchTrigger?.languageCode],
    queryFn: () =>
      getCityKeywordSuggestions({
        data: {
          keyword: baseKeyword,
  ...
  const isLoading = packQuery.isLoading || suggestionsQuery.isLoading || cityKeywordsQuery.isLoading;
  ...
  const analysis = useMemo(() => {
    ...
    const maxReviews = Math.max(0, ...withReviews.map((r) => r.reviewCount ?? 0));
    const minReviews = withReviews.length > 0 ? Math.min(...withReviews.map((r) => r.reviewCount ?? 0)) : 0;

    // --- Rating ---
    const withRating = packResults.filter((r) => r.rating != null);
    const avgRatingNum = withRating.length > 0
      ? withRating.reduce((s, r) => s + (r.rating ?? 0), 0) / withRating.length
      : 0;
    const ratingsAbove4_5 = withRating.filter((r) => (r.rating ?? 0) >= 4.5).length;
    ...
    // --- Fotos ---
    const withPhotos = packResults.filter((r) => r.totalPhotos != null && r.totalPhotos > 0);
    const avgPhotos = withPhotos.length > 0
      ? Math.round(withPhotos.reduce((s, r) => s + (r.totalPhotos ?? 0), 0) / withPhotos.length)
      : 0;
    const maxPhotos = Math.max(0, ...withPhotos.map((r) => r.totalPhotos ?? 0));
    const pctWithPhotos = Math.round((withPhotos.length / total) * 100);

    // --- Verificados ---
    const pctClaimed = Math.round((packResults.filter((r) => r.isClaimed === true).length / total) * 100);

    // --- Con web / contacto / reserva ---
    const pctWithWeb = Math.round((packResults.filter((r) => r.url).length / total) * 100);
    const pctWithContact = Math.round((packResults.filter((r) => r.contactUrl).length / total) * 100);
    const pctWithBooking = Math.round((packResults.filter((r) => r.bookOnlineUrl).length / total) * 100);
    ...
    const withHours = packResults.filter((r) => r.workHours && Object.keys(r.workHours).length > 0).length;
    const pctWithHours = Math.round((withHours / total) * 100);

    const dayOrder = ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"];
    const daysOpen = dayOrder
      .filter((d) => d in dayOpenCounts)
      .map((d) => ({ day: d, count: dayOpenCounts[d] ?? 0, pct: Math.round(((dayOpenCounts[d] ?? 0) / withHours) * 100) }));

    const avgOpen = openHours.length > 0 ? Math.round(openHours.reduce((s, v) => s + v, 0) / openHours.length) : null;
    const avgClose = closeHours.length > 0 ? Math.round(closeHours.reduce((s, v) => s + v, 0) / closeHours.length) : null;
    ...
    return {
      total,
      avgReviewCount, medianReviews, maxReviews, minReviews, totalReviews,
      avgRatingNum, ratingsAbove4_5,
      categoriesRanked,
      avgPhotos, maxPhotos, pctWithPhotos,
      pctClaimed, pctWithWeb, pctWithContact, pctWithBooking,
      priceDistribution,
      pctWithHours, daysOpen, avgOpen, avgClose, formatMinutes,
      topJustifications,
    };
  }, [packResults]);
  ...
  const handleExportCityKeywordsCsv = () => {
    ...
    const headers = ["Keyword", "Volumen", "CPC", "Dificultad", "Intent", "Prioridad"];
    const csvRows = cityKeywords.map((k) => {
      const score = calculatePriorityScore(k.searchVolume, k.keywordDifficulty, k.cpc);
      return [
        csvEscape(k.keyword),
        k.searchVolume ?? "",
        k.cpc?.toFixed(2) ?? "",
      ...
      <div className="shrink-0 px-4 md:px-6 pt-4 pb-2 max-w-8xl mx-auto w-full">
        <div>
          ...
          <p className="text-sm text-base-content/60">
            Analiza el Local Pack de Google Maps: web, contacto, categorías, horarios y más.
          </p>
        </div>
          ...
          {!searchTrigger ? (
            <div className="mt-12 text-center space-y-3">
              ...
              <p className="text-sm text-base-content/50 max-w-md mx-auto">
                ...
                Google Maps con datos completos: web, teléfono, categorías, horarios y ficha de Google.
              </p>
            </div>
          ) : isLoading ? (
            ...
            <>
              ...
              <div className="tabs tabs-bordered mt-4">
                ...
                >
                  ...
                  Keywords Locales ({keywordSuggestions.length + cityKeywords.length})
                </button>
              </div>
                ...
                <div className="mt-3 space-y-4">
                    ...
                    <div className="p-4">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="text-center">
                          <p className="text-2xl font-bold text-warning">{analysis.avgRatingNum.toFixed(1)}</p>
                          <p className="text-xs text-base-content/50">Rating medio</p>
                        </div>
                        <div className="text-center">
                          <p className="text-2xl font-bold">{formatNumber(analysis.avgReviewCount)}</p>
                          <p className="text-xs text-base-content/50">Media reseñas</p>
                        </div>
                        <div className="text-center">
                          <p className="text-2xl font-bold text-info">{formatNumber(analysis.medianReviews)}</p>
                          <p className="text-xs text-base-content/50">Mediana reseñas</p>
                        </div>
                        <div className="text-center">
                          <p className="text-2xl font-bold text-success">{Math.round((analysis.ratingsAbove4_5 / analysis.total) * 100)}%</p>
                          <p className="text-xs text-base-content/50">Con 4.5+ estrellas</p>
                        </div>
                      </div>
                      <div className="mt-3 flex items-center gap-4 text-xs text-base-content/50">
                        <span>Mín: {formatNumber(analysis.minReviews)} reseñas</span>
                        <span>Máx: {formatNumber(analysis.maxReviews)} reseñas</span>
                        <span>Total: {formatNumber(analysis.totalReviews)} reseñas en la zona</span>
                      </div>
                      <div className="mt-3 p-3 bg-primary/5 border border-primary/20 rounded-lg">
                        ...
                        <p className="text-xs text-base-content/70 mt-1">
                          Para estar al nivel de la competencia necesitas al menos <strong>{formatNumber(analysis.medianReviews)} reseñas</strong> con un rating de <strong>{Math.max(analysis.avgRatingNum, 4.0).toFixed(1)}+</strong>. El líder tiene {formatNumber(analysis.maxReviews)} reseñas.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border border-base-300 rounded-xl bg-base-100 overflow-hidden">
                    <div className="px-4 py-3 border-b border-base-300 bg-base-200/30">
                      ...
                      <p className="text-xs text-base-content/50 mt-0.5">
                        Qué categorías de Google Business usan los negocios de la zona
                      </p>
                    </div>
                    <div className="p-4">
                      <div className="space-y-2">
                        {analysis.categoriesRanked.map((cat, i) => (
                          <div key={cat.name} className="flex items-center gap-3">
                            <span className="text-xs text-base-content/40 w-5 text-right tabular-nums">{i + 1}</span>
                            <div className="flex-1">
                              <div className="flex items-center justify-between mb-0.5">
                                <span className="text-sm">{cat.name}</span>
                                <span className="text-xs text-base-content/50 tabular-nums">{cat.count}/{analysis.total} ({cat.pct}%)</span>
                              </div>
                              <div className="w-full bg-base-200 rounded-full h-1.5">
                                <div className="bg-secondary/60 h-1.5 rounded-full" style={{ width: `${cat.pct}%` }} />
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      {analysis.categoriesRanked.length > 0 && (
                        <div className="mt-3 p-3 bg-primary/5 border border-primary/20 rounded-lg">
                          ...
                          <p className="text-xs text-base-content/70 mt-1">
                            La categoría principal más usada es <strong>&quot;{analysis.categoriesRanked[0].name}&quot;</strong> ({analysis.categoriesRanked[0].pct}%). Usa esta como categoría principal y añade las secundarias más relevantes para maximizar tu visibilidad.
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="border border-base-300 rounded-xl bg-base-100 overflow-hidden">
                    ...
                    <div className="p-4">
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {[
                          { label: "Ficha verificada", pct: analysis.pctClaimed, icon: ShieldCheck, color: "text-success" },
                          { label: "Con web", pct: analysis.pctWithWeb, icon: Globe, color: "text-info" },
                          { label: "Con pág. contacto", pct: analysis.pctWithContact, icon: Mail, color: "text-primary" },
                          { label: "Con reserva online", pct: analysis.pctWithBooking, icon: CalendarCheck, color: "text-secondary" },
                          { label: "Con fotos", pct: analysis.pctWithPhotos, icon: Camera, color: "text-warning" },
                          { label: "Con horarios", pct: analysis.pctWithHours, icon: Clock, color: "text-accent" },
                        ].map((item) => (
                          <div key={item.label} className="flex items-center gap-3 p-2 border border-base-200 rounded-lg">
                            <div className={`shrink-0 ${item.color}`}>
                              <item.icon className="size-5" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between">
                                <span className="text-xs text-base-content/60 truncate">{item.label}</span>
                                <span className="text-sm font-bold tabular-nums">{item.pct}%</span>
                              </div>
                              <div className="w-full bg-base-200 rounded-full h-1 mt-1">
                                <div className={`h-1 rounded-full ${item.pct >= 80 ? "bg-success" : item.pct >= 50 ? "bg-warning" : "bg-error"}`} style={{ width: `${item.pct}%` }} />
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="mt-3 p-3 bg-primary/5 border border-primary/20 rounded-lg">
                        ...
                        <div className="mt-1 space-y-0.5">
                          {[
                            { done: true, text: "Verificar la ficha de Google Business" },
                            { done: analysis.pctWithWeb >= 50, text: `Añadir web (${analysis.pctWithWeb}% la tienen)` },
                            { done: analysis.pctWithContact >= 30, text: `Añadir página de contacto (${analysis.pctWithContact}% la tienen)` },
                            { done: analysis.pctWithPhotos >= 50, text: `Subir fotos — media: ${analysis.avgPhotos}, líder: ${formatNumber(analysis.maxPhotos)}` },
                            { done: analysis.pctWithHours >= 50, text: `Publicar horarios (${analysis.pctWithHours}% los tienen)` },
                          ].map((item, i) => (
                            <div key={i} className="flex items-center gap-1.5 text-xs text-base-content/70">
                              {item.done ? (
                  ...
                  <div className="border border-base-300 rounded-xl bg-base-100 overflow-hidden">
                    ...
                    <div className="p-4">
                      <div className="grid grid-cols-3 gap-4 text-center">
                        <div>
                          <p className="text-2xl font-bold">{analysis.avgPhotos}</p>
                          <p className="text-xs text-base-content/50">Media fotos</p>
                        </div>
                        <div>
                          <p className="text-2xl font-bold text-warning">{formatNumber(analysis.maxPhotos)}</p>
                          <p className="text-xs text-base-content/50">Máximo</p>
                        </div>
                        <div>
                          <p className="text-2xl font-bold text-info">{analysis.pctWithPhotos}%</p>
                          <p className="text-xs text-base-content/50">Tienen fotos</p>
                        </div>
                      </div>
                      <div className="mt-3 p-3 bg-primary/5 border border-primary/20 rounded-lg">
                        <p className="text-xs text-base-content/70">
                          Sube al menos <strong>{Math.max(analysis.avgPhotos, 10)} fotos</strong> de calidad (fachada, interior, equipo, trabajos realizados) para superar la media de la zona.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {analysis.daysOpen.length > 0 && (
                    <div className="border border-base-300 rounded-xl bg-base-100 overflow-hidden">
                      <div className="px-4 py-3 border-b border-base-300 bg-base-200/30">
                        ...
                        <p className="text-xs text-base-content/50 mt-0.5">
                          Días y horas habituales de apertura ({analysis.pctWithHours}% publican horarios)
                        </p>
                      </div>
                      <div className="p-4">
                        <div className="space-y-1.5">
                          {analysis.daysOpen.map((d) => (
                            <div key={d.day} className="flex items-center gap-3">
                              <span className="text-xs w-20 text-base-content/60">{DAY_NAMES[d.day] ?? d.day}</span>
                              <div className="flex-1">
                                <div className="w-full bg-base-200 rounded-full h-2">
                                  <div className="bg-accent/60 h-2 rounded-full" style={{ width: `${d.pct}%` }} />
                                </div>
                              </div>
                              <span className="text-xs text-base-content/50 tabular-nums w-16 text-right">{d.count} ({d.pct}%)</span>
                            </div>
                          ))}
                        </div>
                        {analysis.avgOpen != null && analysis.avgClose != null && (
                          <div className="mt-3 flex gap-4 text-sm">
                            <span className="text-base-content/60">Apertura media: <strong>{analysis.formatMinutes(analysis.avgOpen)}</strong></span>
                            <span className="text-base-content/60">Cierre medio: <strong>{analysis.formatMinutes(analysis.avgClose)}</strong></span>
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="p-4">
                        <div className="flex gap-3">
                          {analysis.priceDistribution.map((p) => (
                            <div key={p.level} className="flex-1 text-center p-2 border border-base-200 rounded-lg">
                              <p className="text-lg font-bold">{p.label}</p>
                              <p className="text-xs text-base-content/50">{p.count} negocios ({p.pct}%)</p>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  {analysis.topJustifications.length > 0 && (
                    <div className="border border-base-300 rounded-xl bg-base-100 overflow-hidden">
                      <div className="px-4 py-3 border-b border-base-300 bg-base-200/30">
                        ...
                        <p className="text-xs text-base-content/50 mt-0.5">
                          Razones que Google muestra para recomendar estos negocios
                        </p>
                      </div>
                      <div className="p-4">
                        <div className="flex flex-wrap gap-2">
                          {analysis.topJustifications.map((j, i) => (
                            <span key={i} className="badge badge-outline badge-sm gap-1">
                              {j.text}
                              <span className="text-base-content/40">×{j.count}</span>
                            </span>
                          ))}
                        </div>
                      </div>
                ...
                <>
                  ...
                  {detectedCity && (
                    <div className="flex items-center gap-2 mt-3">
                      <div className="join">
                        ...
                        >
                          Sin ciudad ({cityKeywordsQuery.isLoading ? "..." : cityKeywords.length})
                        </button>
                      </div>
                      <span className="text-xs text-base-content/50">
                        📍 Ciudad detectada: <span className="capitalize font-medium">{cityFound ?? detectedCity}</span>
                        {kwSubTab === "withoutCity" && (
                          <> — Buscando «{baseKeyword}» ubicado en {cityFound ?? detectedCity} (Google Ads)</>
                        )}
                      </span>
                    </div>
                  )}
                  ...
                  {kwSubTab === "withCity" && (
                    <>
                      {suggestionsQuery.isLoading ? (
                        ...
                        <div className="mt-8 text-center text-base-content/50 text-sm">
                          No se encontraron sugerencias de keywords para esta búsqueda.
                        </div>
                      ) : (
                        <KeywordTable
                  ...
                  {kwSubTab === "withoutCity" && (
                    <>
                      {!detectedCity ? (
                        <div className="mt-8 text-center text-base-content/50 text-sm">
                          No se ha detectado la ciudad. Usa el formato «servicio en ciudad» (ej: pintor en terrassa).
                        </div>
                        ...
                        <div className="mt-8 text-center text-base-content/50 text-sm">
                          No se encontraron keywords para «{baseKeyword}» en {cityFound ?? detectedCity}.
                        </div>
                      ) : (
                        <KeywordTable
